...
# Avoid importing TensorFlow through transformers/sentence-transformers in the
# local DDR environment. The pipeline uses PyTorch for embeddings.
...
BASE_DATA_DIR = repo_root / "data" / "processed"
...
def l2_normalize(x: np.ndarray, eps: float = 1e-12) -> np.ndarray:
    # FAISS IndexFlatIP: L2-normalised vectors make inner product equal cosine similarity
...
def _to_int_if_whole(x: Any) -> Optional[int]:
    # Parquet readers may deliver page numbers as int, float (2.0), or str ("2"); normalise all
    if x is None:
        return None
...
def build_meta_table(chunks: pd.DataFrame) -> pd.DataFrame:
    # Retrieval evaluation needs stable doc/page grounding; multi-doc indexing needs globally unique chunk ids
