...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--doc",
        required=True,
        help="Document id, e.g. 30-07a-rb-2024-04-14",
    )
    parser.add_argument(
        "--question",
        required=True,
...
def main() -> None:
    args = parse_args()
    # Prefer local model directory; fall back to HuggingFace Hub name (uses cache)
    _local_model = REPO_ROOT / "models" / "all-MiniLM-L6-v2"
    model_path = _local_model if _local_model.exists() else Path("sentence-transformers/all-MiniLM-L6-v2")
