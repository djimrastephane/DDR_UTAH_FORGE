...
def _resolve_doc_id(doc_id_override: str | None, pdf_path: Path) -> str:
    value = str(doc_id_override or "").strip()
    if not value:
        return pdf_path.stem
    if not DOC_ID_RE.fullmatch(value):
        raise ValueError(
            "Invalid --doc-id. Use only letters, numbers, dots, underscores, and hyphens; "
...
def parse_args() -> argparse.Namespace:
    default_pdf_path = (
        repo_root
        / "data/raw/example_daily_drilling_report.pdf"
    )
    default_out_root = repo_root / "data" / "processed"
    ...
    parser.add_argument(
        "--doc-id",
        default=_env_or_default("DOC_ID", ""),
        help="Optional safe document id override for the output folder and chunk metadata.",
    )
    parser.add_argument(
        "--out-root",
...
def main() -> None:
    ...
    doc_id = _resolve_doc_id(args.doc_id, cfg.PDF_PATH)
    ...
    with pdfplumber.open(str(cfg.PDF_PATH)) as pdf_plumber:
        ...
        ddr_op_rows = 0
        try:
            from ddr_rag.ddr_extractor import run_ddr_extraction
            print("\nRunning DDR extraction (header + Op Summary)...")
            ddr_header_df, ddr_ops_df = run_ddr_extraction(
                cfg.PDF_PATH, doc_id, corpus_id, run_date_utc
            )
            ddr_header_df.to_parquet(out_dir / "ddr_header.parquet", index=False)
            ddr_ops_df.to_parquet(out_dir / "ddr_facts.parquet", index=False)
            ddr_header_rows = len(ddr_header_df)
            ddr_op_rows = len(ddr_ops_df)
            print(f"  ddr_header.parquet: {ddr_header_rows} row(s)")
            print(f"  ddr_facts.parquet:  {ddr_op_rows} op summary rows")
        except ImportError:
            print("  ddr_rag.ddr_extractor not available; skipping DDR post-processing.")
        except Exception as _ddr_exc:
            print(f"  DDR post-processing failed (non-fatal): {_ddr_exc}")

        timer.mark("Step 8.5: DDR post-processing")
        ...
        metrics = {
            ...
            "doc_id": doc_id,
            "source_filename": cfg.PDF_PATH.name,
            "source_path": str(cfg.PDF_PATH),
            "corpus_id": corpus_id,
            "report_year": report_year,
            "period_end_date": period_end_date,
            "run_utc": run_utc,
            ...
            "counts": {
                ...
                "ddr_header_rows": ddr_header_rows,
                "ddr_op_rows": ddr_op_rows,
            },
            "derived": {
                "chunks_per_page": (
