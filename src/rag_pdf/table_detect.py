...
def is_graphics_table_like(drawings: list[dict]) -> bool:
    # Intentionally conservative: ignores filled rectangles (common in chart bars),
    # prefers explicit h/v line evidence, downweights curve-dominant pages (plots).
    if not drawings:
        return False
    cfg = TABLE_DETECT_CFG
...
def is_table_like_from_raw_lines(lines: list[str]) -> bool:
    # More lenient than is_table_like(): checks structure before boilerplate removal
    # may have collapsed the layout.
    cfg = TABLE_DETECT_CFG
    ...
    # This fires before the digit-ratio check to catch sparse tables with strong keywords.
