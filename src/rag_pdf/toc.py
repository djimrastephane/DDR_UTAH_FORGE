...
def parse_toc(pages_df: pd.DataFrame, toc_page_indices: list[int]) -> list[dict[str, Any]]:
    # Handles two common ToC layouts: (1) title+page on the same line, (2) title and page number
    # on separate lines. Wrapped multi-line titles are concatenated; unlabeled major headings
    # without an explicit page number have their page inferred from adjacent entries.
...
def build_toc_spans(toc_items: list[dict[str, Any]], offset: int, n_pdf_pages: int) -> list[dict[str, Any]]:
    # Level-1 headings set the active section title; level-2+ headings set the subsection title.
    if not toc_items:
        return []
...
def fuzzy_match_title(
    ...
    # Uses both token-set Jaccard overlap and SequenceMatcher ratio; a match is declared if
    # either score clears its respective threshold.
