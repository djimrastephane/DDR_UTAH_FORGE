"""Shared utility functions used across search and evaluation scripts.

Provides eval-set loading (handling both list and dict schemas), metadata-based
filtering helpers, and doc-id to report-family mapping.
"""
...
def trust_from_doc_id(doc_id: str) -> str:
    """
    Derive a stable report-family id from document id.

    Examples:
    - '30-07a-rb-2024-04-14' -> '30'
    - 'rig-asset-2024-04-14' -> 'rig'
    """
    ...
    return TRUST_CANONICAL_MAP.get(norm, base.replace("_", " ").strip())


def read_eval_items(eval_path: Path) -> list[dict[str, Any]]:
    """Load eval items from a JSON file that may be a plain list or a dict with a 'queries' key."""
