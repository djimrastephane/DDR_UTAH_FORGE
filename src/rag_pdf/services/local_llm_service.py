...
class LocalLLMService:
    ...
    def health_check(self, timeout_seconds: float = 3.0) -> tuple[bool, str]:
        """Return (ok, detail) — quick probe of the Ollama endpoint without running inference."""
        if not self.enabled:
            return False, "Local LLM disabled (LOCAL_LLM_ENABLED=0)"
        url = f"{self.base_url}/api/version"
        req = urllib.request.Request(url=url, method="GET")
        try:
            with urllib.request.urlopen(req, timeout=float(timeout_seconds)):
                pass
